...
function StructureInspector({ selectedPart, selectedConcept, scene, setScene, armedTool, onOpenEquipment }) {
  if (!selectedPart) return <section className="glass inspector-card empty-inspector"><div className="inspector-icon"><Focus size={20}/></div><span className="panel-kicker">ANATOMY INSPECTOR</span><strong>Select a structure</strong><p>Click directly on the 3D body or search by anatomical name. Scenario-affected anatomy is highlighted with a slow yellow pulse.</p>{armedTool && <div className="armed-callout"><Stethoscope size={16}/><span><b>{armedTool.name} armed.</b> Select a target on the body.</span></div>}</section>
